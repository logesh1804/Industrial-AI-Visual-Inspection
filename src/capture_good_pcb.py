...
print("Capturing fresh clean Good PCB from IP camera...")
    ...
    for _ in range(50):
        ...
        if a != -1 and b != -1:
            ...
            if frame is not None:
                save_path = OUT_DIR / "1_real_good_pcb_raw.jpg"
                cv2.imwrite(str(save_path), frame)
                print(f"[SUCCESS] Clean Real Good PCB Captured: {frame.shape} -> {save_path}")
                break
